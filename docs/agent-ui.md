...
| Endpoint | Purpose |
| --- | --- |
| `GET /settings` | current settings: `{"protected": {"mode", "whitelist"}, "harness"}`. `harness` is one of `claude_code_cli`, `codex_desktop`, `claude_code_desktop`, `codex_cli`, and the harness select must offer each — a test pins every value against the bundle |
