...
Run standalone (mimicking the Pearl runner) — see README "Development" for the required `CONNECTION_*` env vars; cwd must contain `ethereum_private_key.txt`, then `uv run python -m connect --password-stdin` (password on stdin; legacy `--password <password>` still works but leaks into `/proc/<pid>/cmdline`).
