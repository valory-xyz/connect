...
- **100% test coverage** on `connect/` (the agent-runtime trees — bundled skills under `connect/assets/skills/` and the shared modules under `connect/assets/lib/` — are omitted: they run in the agent's environment, not the server's, and have their own tests).
...
- `mech-client` is pinned at `0.22.0`, the first release carrying the `deliveries`/`DeliveryResult` shape `connect/mech.py` reads; `0.21.3` returned a bare IPFS directory URL under `delivery_results` instead.
